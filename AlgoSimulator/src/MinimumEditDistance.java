...
public class MinimumEditDistance extends JApplet implements ActionListener {
	Timer timer;
	...
	private final Integer delete = 1;
	private final Integer insert = 2;
	private final Integer replace = 3;
	...
	public void init() {
		...
		jButtonRefresh = new JButton("Clear");
	...
	private void Init() {
		...
		jLabelGridB = new JLabel[len1 + 3][len2 + 3];
		jLabelGridC = new JLabel[len1 + 3][len2 + 3];
	...
	private void DrawTables() {
		...
		for (i = 0; i <= len1; i++) {
			for (j = 0; j <= len2; j++) {
				jLabelGridB[i + 1][j + 1] = new JLabel(" ");
				jLabelGridC[i + 1][j + 1] = new JLabel(" ");
			}
		}
	...
	private void Traves() {
		...
		jLabelGridC[I][J].setText(C[I][J].toString());
		ChangeLabelSytle(jLabelGridC[I][J], Color.RED, Color.WHITE);
		if (I == len1 && J == len2) {
			FinalState(I, J);
		}
